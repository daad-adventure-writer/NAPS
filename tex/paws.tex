...
\author{\copyright\ 2010, 2018-2020 José Manuel Ferrer Ortiz}
...
Cada palabra de vocabulario solamente puede aparecer una vez en la tabla de vocabulario, lo cual impide que se le pueda asignar más de un tipo de palabra de vocabulario a la misma palabra. Pero se pueden definir nombres que puedan actuar también como verbos, cuando la orden no incluye ningún verbo, dándoles un número de palabra menor que 20 \cite[sección VOC]{PawsPC}.
...
Al inicio de la ejecución de la aventura (y cada vez que ésta se reinicia), el intérprete pone todas las banderas a su valor inicial, salvo en las primeras versiones de \daad (al menos hasta la versión EGA de Jabato), donde el intérprete sólo inicializa las primeras 248 banderas (de la 0 a la 247), y deja las demás (de la 248 a la 255) sin cambiar.
...
\sistemas{\quill, \paw y primeras versiones de \daad}
...
Estas banderas son decrementadas en una unidad por el intérprete cada vez que transcurre un turno de juego.

\subsection{Bandera 9}

\sistemas{\quill, \paw y \daad}

Esta bandera es decrementada en una unidad por el intérprete cada vez que transcurre un turno de juego, cuando el jugador está en un sitio no iluminado (cuando la bandera 0 está a un valor distinto de cero).

\subsection{Bandera 10}

\sistemas{\quill, \paw y \daad}

Esta bandera es decrementada en una unidad por el intérprete cada vez que transcurre un turno de juego, cuando el jugador está en un sitio no iluminado (cuando la bandera 0 está a un valor distinto de cero) y además el objeto 0 (fuente de luz) está ausente.
...
Contiene el primer verbo de la sentencia lógica actual.
...
Esta bandera, inicialmente documentada como reservada para futuras ampliaciones, fue utilizada en las versiones de PAWS A16 y posteriores, como parte del soporte para dar órdenes a los PSI. \cite{PawsSupl}
...
Estas banderas conservan su valor incluso al reiniciar la aventura, en las primeras versiones de \daad (al menos hasta la versión EGA de Jabato).
...
Las condiciones son condactos que pueden interrumpir el flujo de ejecución,
...
Las acciones son condactos que no interrumpen el flujo de ejecución.
...
Comprobado con \daad, desde la tabla de proceso 1 no se permite llamar ni al proceso 0 ni al 2, ni a un número de proceso inexistente (en todos estos casos da error 6).
...
\sistemas{\paw y primeras versiones de \daad}
...
Esta tabla de procesos es la tabla de respuestas en \paw y las primeras versiones de \daad (al menos hasta la versión EGA de Jabato). En las versiones de \daad posteriores, esta tabla es el bucle principal de la aventura.
